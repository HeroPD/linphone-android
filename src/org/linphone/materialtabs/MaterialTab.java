package org.linphone.materialtabs;
...
public class MaterialTab implements View.OnTouchListener {
	...
	public MaterialTab(Context ctx, boolean hasIcon) {
		...
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			if (!hasIcon) {
				completeView = LayoutInflater.from(ctx).inflate(R.layout.tab,
						null);

				text = (TextView) completeView.findViewById(R.id.text);
			} else {
				completeView = LayoutInflater.from(ctx).inflate(
						R.layout.tab_icon, null);
			...
			if (!hasIcon) {
				// if there is no icon
				completeView = LayoutInflater.from(ctx).inflate(
						R.layout.material_tab, null);

				text = (TextView) completeView.findViewById(R.id.text);
			} else {
				// with icon
				completeView = LayoutInflater.from(ctx).inflate(
						R.layout.material_tab_icon, null);

				icon = (ImageView) completeView.findViewById(R.id.icon);
				notification = (TextView) completeView
	...
	public MaterialTab setText(CharSequence text) {
		if (hasIcon)
			throw new RuntimeException(
					"You had setted tabs with icons, uses icons instead text");
	...
	public MaterialTab setIcon(Drawable icon) {
		if (!hasIcon)
			throw new RuntimeException(
					"You had setted tabs without icons, uses text instead icons");
	...
	public MaterialTab setNotificationText(int text) {
		String tempText = ""+text;
		this.notification.setText(tempText);
		if (text == 0)
			this.notification.setVisibility(View.GONE);
		else
	...
	public void disableTab() {
		...
		if (text != null)
			this.text.setTextColor(Color.argb(0x99, Color.red(textColor),
		...
		this.selector.setBackgroundColor(res
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			...
			if (!deviceHaveRippleSupport()) {
				...
				this.background.reveal(
						lastTouchedPoint.x,
						lastTouchedPoint.y,
						Color.argb(0x80, Color.red(accentColor),
								Color.green(accentColor),
								Color.blue(accentColor)), 0, REVEAL_DURATION,
						new Animator.AnimatorListener() {
							...
							public void onAnimationEnd(Animator animation) {
								background.reveal(lastTouchedPoint.x,
										lastTouchedPoint.y, primaryColor, 0,
										HIDE_DURATION, null);
							}
	...
	private void setIconAlpha(int paramInt) {
		...
		this.icon.setColorFilter(Color.argb(paramInt,
				Color.red(this.iconColor), Color.green(this.iconColor),
